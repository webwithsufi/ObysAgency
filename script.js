...
function cursorAnimation() {
  ...
  function updateCursorPosition(dets) {
    let rect = videoContainer.getBoundingClientRect(); // Get video container position

    gsap.to(videoCursor, {
      left: `${dets.clientX - rect.left}px`, // Position cursor relative to container
      top: `${dets.clientY - rect.top}px`,
      opacity: 1,
      ease: "power2.out",
  ...
  videoContainer.addEventListener("mouseenter", function () {
    gsap.to(".mousefollower", { opacity: 0 }); // Hide default cursor
    gsap.to(videoCursor, { opacity: 1 }); // Show video cursor

    videoContainer.addEventListener("mousemove", updateCursorPosition);
  });

  videoContainer.addEventListener("mouseleave", function () {
    gsap.to(".mousefollower", { opacity: 1 }); // Restore default cursor
    gsap.to(videoCursor, { opacity: 0 }); // Hide video cursor
  });
